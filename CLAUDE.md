...
- 진도 현황 대시보드: 좌측 레일 진도바 아래(`.raildash`, 모바일 숨김). 현재 레벨 완료율 %, "다음 자세"(미완료 첫 자세, 클릭 시 해당 섹션으로 이동, 점 하나가 펄스), 섹션별 `n/총` + 자세 한 개 = 점 하나 그리드(완료 앰버·미완료 회색, hover 시 자세 이름 툴팁). 레벨 탭을 바꾸면 그 레벨 기준으로 갱신. i18n 키 `dashT`/`dashNext`/`dashAll`.
